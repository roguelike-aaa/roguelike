...
from shared.player_map import Player, PlayerToken, MoveType, PlayerMove, StateChange
...
class TestSession(unittest.TestCase):
    ...
    def test_player_simple_move(self):
        ...
        self.assertEqual(Coordinate(1, 3), self.session.players[self.player_token].coordinate)

    def test_player_move_denied(self):
        self.session.change_player_state(self.player_token, StateChange(PlayerMove(MoveType.DOWN)))
        self.assertEqual(Coordinate(1, 2), self.session.players[self.player_token].coordinate)

    def test_player_move_on_door(self):
        self.session.change_player_state(self.player_token, StateChange(PlayerMove(MoveType.LEFT)))
        self.assertEqual(Coordinate(1, 1), self.session.players[self.player_token].coordinate)
    ...
    def test_player_move_on_path(self):
        self.session.change_player_state(self.player_token, StateChange(PlayerMove(MoveType.LEFT)))
        self.session.change_player_state(self.player_token, StateChange(PlayerMove(MoveType.LEFT)))
        self.assertEqual(Coordinate(1, 0), self.session.players[self.player_token].coordinate)
